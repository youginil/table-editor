...
export type TdRange = [number, number];
type TdData = {
  row: TdRange;
  col: TdRange;
  content: string;
...
export type TableData = Array<TrData>;
...
export type TableCells = Array<TdData>;

export function tdRangeToString(range: TdRange | Array<TdRange>): string {
  if (range.length === 0) {
    return '';
  }
...
export class Tr {
  ...
  addTd(td: Td): number {
    ...
    if (
      this.tds.length === 0 ||
      this.tds[this.tds.length - 1].getColRange()[1] < colRange[0]
    ) {
      // 空行或者需要插入到最后
      this.elem.appendChild(td.getElem());
      this.tds.push(td);
      ...
      for (let i = 0; i < this.tds.length; i++) {
        if (i !== this.tds.length - 1) {
          ...
          if (
            colRange[0] > tmpColRange[1] &&
            colRange[1] < tmpNextColRange[0]
          ) {
            // 插入到空洞
    ...
    log.error(
      'Tr.addTd',
      `Add td fail. tds col range: ${tdRangeToString(
        tdsRange
      )}, tdRange: ${tdRangeToString(colRange)}`
    );
    return 0;
  }
    ...
    for (let i = 0; i < this.tds.length; i++) {
      ...
      if (colIdx >= holeStart && colIdx < colRange[0]) {
        ...
        for (let j = i; j < this.tds.length; j++) {
          ...
          tmpTd.setColRange([
            tmpColRange[0] + offsetCols,
            tmpColRange[1] + offsetCols,
          ]);
        }
        break;
...
export class Table {
              ...
              props: {
                style:
                  'style' in tdData
                    ? extend(
                        this.cellStyle,
                        tdData.style as { [prop: string]: string }
                      )
                    : this.cellStyle,
              ...
              props: {
                style:
                  'style' in td
                    ? extend(
                        this.cellStyle,
                        td.style as { [prop: string]: string }
                      )
                    : this.cellStyle,
                class: this.cellClass,
              },
  ...
  private initEventListener() {
    ...
    this.elem.addEventListener('mousedown', (e) => {
      ...
      if (
        (e.offsetX < RESIZE_OFFSET && tmpIdx > 0) ||
        e.offsetX > target['clientWidth'] - RESIZE_OFFSET
      ) {
        ...
        const maxLeftOffset =
          +(colEls[tmpIdx + 1] as HTMLElement)['style'].width.slice(0, -2) -
          2 * RESIZE_OFFSET -
          30;
        ...
        if (tmpIdx === colEls.length) {
          ...
          this.resizeRange = [
            maxLeftOffset,
            (this.elem.parentElement as HTMLElement).clientWidth -
              this.elem.clientWidth,
          ];
          this.colElsResizing = [colEl];
        } else {
          const maxRightOffset =
            +colEl['style'].width.slice(0, -2) - 2 * RESIZE_OFFSET - 30;
    ...
    this.elem.addEventListener('mousemove', (e) => {
      ...
      if (this.mouseMode === MouseMode.NONE) {
        if (
          this.eventTargetIsCellContent(e) &&
          (e.offsetX < RESIZE_OFFSET ||
            e.offsetX > target['clientWidth'] - RESIZE_OFFSET)
        ) {
          // 鼠标在竖线上
          if (this.resizeable) {
        ...
        if (this.resizeable) {
          ...
          if (this.colElsResizing.length > 1) {
            ...
            rightColEl.style.width = `${
              // @ts-ignore
              rightColEl['originWidth'] + leftOffset
            }px`;
          }
        }
  ...
  private eventTargetIsCellContent(e: Event) {
    ...
    return (
      ...
      e.target.classList.contains('cell-content') &&
      ep[4] === this.elem
    );
  }
  ...
  getTdsCrossRow(
    ...
    if (fromRowIdx < 0 || fromRowIdx >= this.trs.length) {
      log.error(
        'Table.getTdsCrossRow',
        `Invalid param fromRowIdx: ${fromRowIdx}`
      );
      return ret;
    }
    let c = 1;
  ...
  validate(): string {
    ...
    for (let i = 0; i < this.trs.length; i++) {
      ...
      for (let j = 0; j < tds.length; j++) {
        ...
        const colspan = tdElem.hasAttribute('colspan')
          ? +(tdElem as any).getAttribute('colspan')
          : 1;
        if (colspan !== colRange[1] - colRange[0] + 1) {
          return `Colspan not match. rowIndex: ${i}, colIndex: ${j}`;
        }
        const rowspan = tdElem.hasAttribute('rowspan')
          ? +(tdElem as any).getAttribute('rowspan')
          : 1;
    ...
    for (let tri = 0; tri < trs.length; tri++) {
      ...
      for (let hi = 0; hi < holes.length; hi++) {
        if (
          hi !== holes.length - 1 &&
          holes[hi].range[1] >= holes[hi + 1].range[0]
        ) {
    ...
    this.trs.forEach((tr, rowIdx) => {
      ...
      for (let i = 0; i < tds.length; i++) {
        ...
        if (holeStartIdx < tdColRange[0]) {
          // 把空洞中所有的点取出来
          const holes = this.getTdsCrossRow(
            rowIdx,
            holeStartIdx,
            tdColRange[0] - 1
          );
          for (let j = 0; j < holes.length; j++) {
            ...
            if (
              tmpHoleTdColRange[0] > startIdx &&
              tmpHoleTdColRange[0] < endIdx
            ) {
              points.add(tmpHoleTdColRange[0]);
            }
            if (
              tmpHoleTdColRange[1] + 1 > startIdx &&
              tmpHoleTdColRange[1] + 1 < endIdx
            ) {
