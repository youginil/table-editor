...
export default class TableEditor {
  ...
  constructor(options: EditorOptions) {
    ...
    this.table = new Table({
      ...
      cellStyle:
        'cellStyle' in options
          ? (options.cellStyle as { [prop: string]: string })
          : {},
    ...
    this.cmdHistory = new CommandHistory(
      'maxUndoTimes' in options && (options.maxUndoTimes as number) > 0
        ? (options.maxUndoTimes as number)
        : 10
    );
  }
  ...
  mergeCells(rowRange: TdRange, colRange: TdRange): boolean {
    ...
    if (this.debug) {
      log.info(
        `Merge cells. Row: ${rowRange[0]} ~ ${rowRange[1]}, Column: ${colRange[0]} ~ ${colRange[1]}`
      );
    }
  ...
  splitCell(
    ...
    if (this.debug) {
      log.info(
        `Split cell (${rowIdx}, ${colIdx}) into ${rowCount} rows and ${colCount} columns`
      );
    }
    const cmd = new CmdSplitCell(
      this.table,
      rowIdx,
      colIdx,
      rowCount,
...
class CommandHistory {
  ...
  printStatus() {
    log.info(
      `👽 Command History: Capability: ${this.cap}  Undo: ${
        this.divide
      }  Redo: ${this.top - this.divide}`,
      this.commands
    );
  }
}
