...
class CmdAddCell implements Command {
  ...
  execute(): boolean {
    ...
    if (!tr) {
      log.error(
        'CmdAddCell',
        `Invalid rowRange: ${rowRange}, colRange: ${this.td.getColRange()}`
      );
      return false;
    }
  ...
  undo(): boolean {
    return (
      this.table.removeCell(
        this.td.getRowRange()[0],
        this.td.getColRange()[0]
      ) > 0
    );
  }
}
...
class CmdDelCell implements Command {
  ...
  execute(): boolean {
    ...
    if (!this.td) {
      log.error(
        'CmdDelCell',
        `Cell not found. rowIdx: ${this.rowIdx}, colIdx: ${this.colIdx}`
      );
      return false;
    }
...
class CmdSetCellRowRange implements Command {
  ...
  execute(): boolean {
    ...
    if (!td) {
      log.error(
        'CmdSetCellRowRange',
        `Invalid position. rowIdx: ${this.rowIdx}, colIdx: ${this.colIdx}`
      );
      return false;
    }
...
class CmdSetCellColRange implements Command {
  ...
  undo(): boolean {
    ...
    if (!td) {
      log.error(
        'CmdSetCellColRange',
        `Cell not found. rowIdx: ${this.newRange[0]}, colIdx: ${this.newRange[1]}`
      );
      return false;
    }
...
export class CmdAddRow implements Command {
  ...
  execute(): boolean {
    ...
    if (this.refRowIdx !== trs.length - 1 || this.above) {
      const rowIdxWillMoveDown = this.above
        ? this.refRowIdx + 1
        : this.refRowIdx + 2;
      this.cmdMacro.addCommand(
        new CmdMoveRow(this.table, rowIdxWillMoveDown, 1)
      );
    }
    ...
    for (let i = 0; i < relativeTds.length; i++) {
      ...
      if (relTdRowRange[0] === relTdRowRange[1] || this.above) {
        ...
        this.cmdMacro.addCommand(
          new CmdSetCellRowRange(
            this.table,
            relTdRowRange[0],
            relTdColRange[0],
            [relTdRowRange[0], relTdRowRange[1] + 1]
          )
        );
      }
      // 前面有空洞
      if (holeStartColIdx < relTdColRange[0]) {
        ...
        // 就把跨行的单元格增加跨一行
        const tmpTds: Array<Td> = this.table.getTdsCrossRow(
          this.refRowIdx - 1,
          holeStartColIdx,
          relTdColRange[0] - 1
        );
    ...
    if (holeStartColIdx < lastColIdx) {
      const tmpTds = this.table.getTdsCrossRow(
        this.refRowIdx - 1,
        holeStartColIdx,
        lastColIdx
      );
...
export class CmdDelRow implements Command {
  ...
  execute(): boolean {
    ...
    tr.getTds().forEach((td) => {
      this.cmdMacro.addCommand(
        new CmdDelCell(this.table, td.getRowRange()[0], td.getColRange()[0])
      );
    });
...
export class CmdAddColumn implements Command {
  ...
  execute(): boolean {
    ...
    this.cmdMacro.addCommand(
      new CmdAddColHeader(
        this.table,
        this.left ? this.refColIdx : this.refColIdx + 1
      )
    );
    ...
    for (let i = 0; i < trs.length; i++) {
      ...
      for (let j = 0; j < tds.length; j++) {
        ...
        if (
          holeStart < colRange[0] &&
          this.refColIdx === holeStart &&
          this.refColIdx <= colRange[0] - 1
        ) {
          this.cmdMacro.addCommand(
            new CmdMoveCol(this.table, i, colRange[0], 1)
          );
          break;
        }
        // 参考位置在单元格中
        if (this.refColIdx >= colRange[0] && this.refColIdx <= colRange[1]) {
          ...
          if (
            ...
            this.cmdMacro.addCommand(
              new CmdSetCellColRange(this.table, rowRange[0], colRange[0], [
                colRange[0],
                colRange[1] + 1,
              ])
            );
            ...
            this.cmdMacro.addCommand(
              new CmdMoveCol(this.table, rowRange[0], tmpColIdx, 1)
            );
            const tmpTd = this.table.createCell({
        ...
        if (
          j === tds.length - 1 &&
          colRange[1] < colCount - 1 &&
          this.refColIdx > colRange[1] &&
          this.refColIdx <= colCount - 1
        ) {
          // do nothing
        }
  ...
  execute(): boolean {
    ...
    trs.forEach((tr, i) => {
      ...
      tdsMatched.forEach((td) => {
        this.cmdMacro.addCommand(
          new CmdDelCell(this.table, i, td.getColRange()[0])
        );
      });
      this.cmdMacro.addCommand(
        new CmdMoveCol(this.table, i, this.colIdx, -offset)
      );
    });
    const c = this.table.getColCount();
    this.cmdMacro.addCommand(new CmdSetColCount(this.table, c - 1));
...
class CmdMoveRow implements Command {
  ...
  execute(): boolean {
    ...
    if (this.rowIdx < 0 || this.rowIdx >= trs.length) {
      log.error(
        'CmdMoveRow',
        `Invalid rowIdx: ${this.rowIdx}, total rows: ${trs.length}`
      );
      return false;
    }
  ...
  execute(): boolean {
    ...
    for (let i = 0; i < tds.length; i++) {
      ...
      if (holeStart < colRange[0]) {
        ...
        const rowRange = td.getRowRange();
        this.cmdMacro.addCommand(
          new CmdSetCellRowRange(this.table, rowRange[0], colRange[0], [
            rowRange[0],
            rowRange[1] + this.offsetRows,
          ])
        );
      }
      holeStart = colRange[1] + 1;
    ...
    holeList.forEach((holeRange) => {
      const holes = this.table.getTdsCrossRow(
        this.rowIdx,
        holeRange[0],
        holeRange[1]
      );
      holes.forEach((hole) => {
        ...
        this.cmdMacro.addCommand(
          new CmdSetCellRowRange(this.table, holeRowRange[0], holeColRange[0], [
            holeRowRange[0],
            holeRowRange[1] + this.offsetRows,
          ])
        );
      });
    });
...
export class CmdMergeCells implements Command {
  ...
  execute(): boolean {
    ...
    if (
      rowRange[0] < 0 ||
      rowRange[0] >= trs.length ||
      rowRange[1] < 0 ||
      rowRange[1] >= trs.length ||
    ...
    if (
      colRange[0] < 0 ||
      colRange[0] >= totalColCount ||
      colRange[1] < 0 ||
      colRange[1] >= totalColCount ||
      colRange[0] > colRange[1]
    ) {
    ...
    for (let i = rowRange[0]; i <= rowRange[1]; i++) {
      const tr = trs[i];
      if (
        !tr.leftAlign(colRange[0], true) ||
        !tr.rightAlign(colRange[1], true)
      ) {
        log.error(
          'Table.mergeCells',
          `Invalid range. rowRange: ${rowRange}, colRange: ${colRange}`
        );
        return false;
      }
    }
    ...
    for (let i = 0; i < tmpTds.length; i++) {
      ...
      if (holeStartIdx < tmpColRange[0]) {
        const tdsInHole = this.table.getTdsCrossRow(
          rowRange[1],
          holeStartIdx,
          tmpColRange[0] - 1
        );
        if (tdsInHole.length === 0) {
          log.error(
            'CmdMergeCells',
            `Invalid hole. rowIndex: ${rowRange[1]}, colRange: [${
              colRange[0]
            }, ${tmpColRange[0] - 1}]`
          );
          return false;
    ...
    this.cmdMacro.addCommand(
      new CmdAddCell(this.table, tdMerged),
      new CmdRemoveBlankRows(this.table)
    );
...
class CmdShrinkColumns implements Command {
  ...
  execute(): boolean {
      ...
      for (let j = 0; j < trs.length; j++) {
        ...
        for (let z = 0; z < tds.length; z++) {
          ...
          if (colRange[0] <= insRange[0] && colRange[1] >= insRange[1]) {
            ...
            const tmpColIdxAfterShrink =
              colRange[0] +
              (colRange[1] - colRange[0]) -
              (insRange[1] - insRange[0]) -
              1;
            cmdList.push(
              new CmdSetCellColRange(this.table, j, colRange[0], [
                colRange[0],
                tmpColIdxAfterShrink,
              ])
            );
            if (z !== tds.length - 1) {
              // 把后面的单元格往左移动
            ...
            cmdList.push(
              new CmdMoveCol(this.table, j, colRange[0], -insRangeCount)
            );
            break;
          }
          // 交集在最后的空洞里不需要做任何事
    ...
    this.cmdMacro.addCommand(
      new CmdSetColCount(
        this.table,
        this.table.getColCount() - colCountShrinked
      )
    );
    return this.cmdMacro.execute();
  }
...
export class CmdSplitCell implements Command {
  ...
  execute(): boolean {
    ...
    // 拆分必须是整数倍
    if (
      this.rowCount % originRowCount !== 0 &&
      originRowCount % this.rowCount !== 0
    ) {
      log.error(
        `${originRowCount} rows cannot be split into ${this.rowCount} rows`
      );
      return false;
    }
    if (
      this.colCount % originColCount !== 0 &&
      originColCount % this.colCount !== 0
    ) {
      log.error(
        `${originColCount} columns cannot be split into ${this.colCount} columns`
      );
      return false;
    }
    // *** 分割行 ***
    ...
    if (this.rowCount > originRowCount) {
      ...
      for (let i = originStartRowIdx; i < originEndRowIdx + 1; i++) {
        ...
        this.cmdMacro.addCommand(
          new CmdExtendRows(this.table, tmpIdx, blankRowsInc)
        );
        // 把被拆分单元格所在的所有行向下扩展
    ...
    // 缩小被拆分单元格所占的行数
    this.cmdMacro.addCommand(
      new CmdSetCellRowRange(this.table, originStartRowIdx, originStartColIdx, [
        originStartRowIdx,
        originStartRowIdx + rowStep - 1,
      ])
    );
    // 插入单元格到被拆分的单元格中
    for (
      let i = originStartRowIdx + rowStep;
      i < endRowIdxAfterSplit + 1;
      i += rowStep
    ) {
      const tmpTd = this.table.createCell({
    ...
    if (this.colCount > originColCount) {
      ...
      for (let i = originStartColIdx; i < originEndColIdx + 1; i++) {
        for (let j = 0; j < finalEndRowIdx; j++) {
          this.cmdMacro.addCommand(
            new CmdExtendCols(this.table, j, originStartColIdx, blankColsInc)
          );
        }
    ...
    for (let i = originStartRowIdx; i < endRowIdxAfterSplit + 1; i += rowStep) {
      // 缩小被拆分的单元格
      this.cmdMacro.addCommand(
        new CmdSetCellColRange(this.table, i, originStartColIdx, [
          originStartColIdx,
          originStartColIdx + colStep - 1,
        ])
      );
      // 插入新增的单元格
      for (
        let j = originStartColIdx + colStep;
        j < endColIdxAfterSplit + 1;
        j += colStep
      ) {
        const tmpTd = this.table.createCell({
          rowRange: [i, i + rowStep - 1],
    ...
    this.cmdMacro.addCommand(
      ...
      new CmdSetColCount(
        this.table,
        this.table.getColCount() + this.colCount - 1
      )
    );
    return this.cmdMacro.execute();
  }
